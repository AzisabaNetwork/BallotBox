...
import net.azisaba.ballotbox.server.connection.QuietException;
...
public class VotifierProtocolDifferentiator extends ByteToMessageDecoder {
  private static final QuietException V2_ONLY =
      new QuietException("This server only accepts well-formed Votifier v2 packets.");
    ...
    if (readMagic == PROTOCOL_2_MAGIC) {
      ...
      if (!testMode) {
        ...
        ctx.pipeline()
            .addAfter(
                "protocol2LengthDecoder",
                "protocol2StringDecoder",
                new StringDecoder(StandardCharsets.UTF_8));
        ctx.pipeline()
            .addAfter(
                "protocol2StringDecoder", "protocol2VoteDecoder", new VotifierProtocol2Decoder());
        ctx.pipeline()
            .addAfter(
                "protocol2VoteDecoder",
                "protocol2StringEncoder",
      ...
      if (!testMode) {
        ctx.pipeline()
            .addAfter(
                "protocol_differentiator", "protocol1Handler", new VotifierProtocol1Decoder());
        ctx.pipeline().remove(this);
      }
    }
  }
