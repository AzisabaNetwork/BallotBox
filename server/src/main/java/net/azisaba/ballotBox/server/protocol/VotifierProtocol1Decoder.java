...
public class VotifierProtocol1Decoder extends ByteToMessageDecoder {
  ...
  protected void decode(
      ChannelHandlerContext ctx, ByteBuf buf, List<Object> list) /*throws Exception*/ {
    ...
    if (buf.readableBytes() > 256) {
      ...
      throw new QuietException(
          "Could not decrypt data from "
              + ctx.channel().remoteAddress()
              + " as it is too long. Attack?");
    }
    ...
    try {
      block = RSA.decrypt(block, ServerConfigInstance.keyPair.getPrivate());
    } catch (Exception e) {
      if (ServerConfigInstance.debug) {
    ...
    if (split.length < 5) {
      throw new QuietException(
          "Not enough fields specified in vote. This is not a NuVotifier issue. Got "
              + split.length
              + " fields, but needed 5.");
    }
